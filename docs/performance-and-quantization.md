...
- **Llama / Qwen / Qwen3.5 MoE text / Mistral / Gemma / Gemma2 / Gemma3-Text / Phi / Phi3** (`qwen3_5_moe` and `qwen3_5_moe_text` route through the qwen3 streamer), `task: sft`, `backend: transformers`, `modality: text`. The original list is verified bit-exact in bf16 and NF4. Qwen3.5's heterogeneous MoE decoder path is verified bit-exact against a resident toy model on CPU and has live streamed-training validation on `Qwen/Qwen3.5-35B-A3B`; the real 35B run has no resident control because the available hardware could not load it resident.
